var HOST_ROOT = 'http://wiki.tinyxd.me/',
tinywiki_identifier = 'wiki' + window.location.pathname.replace(/\//g, '_').replace('index.html', '').replace('.html', '');

var tinywiki = tinywiki || {};

tinywiki.getCSS = function(url) {
    var tag = '<link rel="stylesheet" href="' + url + '" />';
    $('body').append(tag);
};

tinywiki.insertCSS = function(url) {
...
tinywiki.insertScript = function(url, bottle) {
    var tag = '<script src=' + url + '></script>';
    if (!bottle) bottle = 'head';
    $(bottle).append(tag);
};

tinywiki.loadCmt = function(provider) {
    ...
    if (provider == 'disqus') {
        ...
        window.disqus_identifier = window.tinywiki_identifier || undefined;
        thread = $('<div id="disqus_thread">');
        extraCSS = HOST_ROOT + 'disqus.css';
        //theJS = HOST_ROOT + 'disqus_loader.js';
        theJS = 'http://tinywiki.disqus.com/embed.js';
    } else if (provider == 'intensedebate') {
        window.idcomments_acct = '63u0z6q1wSO9c4imW9pl';
        window.idcomments_post_id = tinywiki_identifier;
    ...
    function load() {
        if (win.scrollTop() + win.height() > thread.offset().top - 100) {
            ...
            if (extraCSS) tinywiki.getCSS(extraCSS);
...
$(document).ready(function() {
    ...
    //评论
    if (!$.browser.msie) tinywiki.loadCmt('disqus');

});

//Google 统计
var _gaq = _gaq || [];
_gaq.push(['_setAccount', 'UA-32291601-4']);
